...
foreach ($groups as $file => $groupName) {
    ...
    if (file_exists($path)) {
        ...
        // Match any navigationGroup definition and replace the value
        $content = preg_replace('/(protected static .*?\$navigationGroup\s*=\s*\').*?(\';)/', "$1$groupName$2", $content);
        
        file_put_contents($path, $content);
        echo "Updated group to $groupName for $file\n";
    }
}
